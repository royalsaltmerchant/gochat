...
// Free tier usage tracking: token -> minutes used today
var freeUsageTracker = map[string]int{}
var freeUsageMu sync.Mutex
var freeUsageLastReset time.Time
...
type CallRoom struct {
	...
	CreatedAt    time.Time
	MaxDuration  time.Duration // 40min for free, 6hr for premium
...
func resetFreeUsageIfNeeded() {
	freeUsageMu.Lock()
	defer freeUsageMu.Unlock()
	now := time.Now()
	if now.Day() != freeUsageLastReset.Day() || now.Month() != freeUsageLastReset.Month() {
		freeUsageTracker = map[string]int{}
		freeUsageLastReset = now
	}
}
