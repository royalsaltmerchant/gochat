...
func HandleInsertSpace(c *gin.Context) {
	...
	query = `INSERT INTO channels (uuid, name, space_id) VALUES (?, ?, ?) RETURNING *`
	err = db.DB.QueryRow(query, channelUUID, initalChannelName, space.ID).Scan(&channel.ID, &channel.UUID, &channel.Name, &channel.SpaceID)
